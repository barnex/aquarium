...
pub fn decode_normal_map(normal_map: &RgbImage) -> Img<vec3f> {
	let mut img = Img::new(normal_map.dimensions().into());

	for (x, y, &Rgb([r, g, b])) in normal_map.enumerate_pixels() {
		let rgb = vec3(r, g, b).map(|v| (v as f32) / 255.0); // map to 0..1
		let normal = 2.0 * (rgb - vec3f(0.5, 0.5, 0.5)); // map to -1..1
...
pub fn encode_normal_map(normal_map: &Img<vec3f>) -> RgbImage {
	let (w, h) = normal_map.size().into();
	RgbImage::from_fn(w, h, |x, y| {
		let normal = normal_map.at((x, y));
		let rgb = 0.5 * (normal + vec3f(1.0, 1.0, 1.0)); // map to 0..1
